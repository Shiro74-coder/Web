...
public class AccountDAO {
    public Account getByUsername(String username) {
        try (Connection con = new database().getConnection()) {
            ...
            if (rs.next()) {
                ...
                try { a.setEmail(rs.getString("email")); } catch(Exception ignore){}
                try { a.setPhone(rs.getString("phone")); } catch(Exception ignore){}
                return a;
            }
    ...
    public void insert(Account a) {
        String sql = "INSERT INTO Account(username,password,fullname,role,email,phone) "
                   + "VALUES(?,?,?,?,?,?)";
        try (Connection con = new database().getConnection();
             PreparedStatement ps = con.prepareStatement(sql)) {
