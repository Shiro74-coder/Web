...
<body class="min-h-screen flex items-center justify-center bg-gray-100">
  <div class="bg-white w-full max-w-md p-6 rounded-xl shadow">
    <h1 class="text-2xl font-semibold mb-2">Quên mật khẩu</h1>
    <p class="text-sm text-gray-500 mb-4">Nhập email và đặt mật khẩu mới.</p>

    <c:if test="${alert != null}">
      <div class="mb-4 p-3 text-sm text-red-700 bg-red-100 rounded">${alert}</div>
    </c:if>

    <form action="${pageContext.request.contextPath}/forgot" method="post" class="space-y-4">
      <div>
      ...
      <button class="w-full bg-indigo-600 hover:bg-indigo-700 text-white py-2 rounded">Cập nhật mật khẩu</button>
    </form>
